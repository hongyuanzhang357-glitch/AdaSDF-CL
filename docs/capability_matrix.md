...
| Build / Input | adaptive build sample cache | Alpha available | v1.17.2 | `--sample-cache`, `--corner-cache`, `--report-cache-stats` | CPU-only build-time cache for repeated SDF sample and corner queries. Uses deterministic quantized point keys and keeps `--sample-cache off` as reference. No `.sdfbin` format change. |
| Build / Input | contact-band marker cache | Alpha available | v1.17.2 | `--marker-cache`, `ContactBandMarkerCache` | Caches marker decisions/candidates using marker parameter config ids. Does not reuse entries across changed marker parameters. |
...
| Benchmark | build cache benchmark | Alpha available | v1.17.2 | `adasdf_benchmark_build_cache` | Compares cache off/block/global modes against a no-cache reference and reports speedup, saved queries, hit rates, phi diff, sign mismatches, quality gate, and performance-claim gate. |
