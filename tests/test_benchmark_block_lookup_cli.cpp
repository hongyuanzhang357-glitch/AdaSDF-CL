...
#endif
#ifndef ADASDF_CL_BENCHMARK_BLOCK_LOOKUP
#define ADASDF_CL_BENCHMARK_BLOCK_LOOKUP ""
#endif
...
int main() {
  ...
  const std::string benchmark_tool = ADASDF_CL_BENCHMARK_BLOCK_LOOKUP;
  if (build_tool.empty() || benchmark_tool.empty()) {
    std::cout << "SKIP: block lookup benchmark CLI tools were not built\n";
    return 0;
  }
  std::filesystem::path temp = ADASDF_CL_TEST_TEMP_DIR;
  std::filesystem::create_directories(temp);
  const auto model = temp / "block_lookup_cli_compressed.sdfbin";
  const auto stdout_txt = temp / "block_lookup_cli_stdout.txt";
  const auto csv = temp / "block_lookup_cli.csv";
  const auto report = temp / "block_lookup_cli_report.md";
  const auto samples =
  ...
  if (std::system((build_cmd + " " + quote(fixture) + " " + quote(model) +
                   ...
                   "--fixed-rank 2 > " +
                   quote(temp / "block_lookup_build.txt"))
                      .c_str()) != 0) {
  ...
  const std::string command =
      ...
      "--cache-lookup linear,hash,spatial-hash --repeat 2 --csv " +
      quote(csv) + " --report " + quote(report) +
      " --case-id test_block_lookup > " + quote(stdout_txt);
  if (std::system(command.c_str()) != 0 ||
      !std::filesystem::exists(csv) ||
      !std::filesystem::exists(report) ||
      !contains(readFile(stdout_txt), "Status: ok") ||
      !contains(readFile(csv), "speedup_vs_linear") ||
      !contains(readFile(csv), "lookup_result_mismatch_count") ||
      !contains(readFile(csv), "performance_claim_allowed")) {
    std::cerr << "block lookup benchmark CLI failed\n";
    return 1;
  }
  std::cout << "block lookup benchmark CLI passed\n";
  return 0;
}
