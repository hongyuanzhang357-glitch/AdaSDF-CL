...
#endif
#ifndef ADASDF_CL_WORLD_SOLVER_CONTACTS
#define ADASDF_CL_WORLD_SOLVER_CONTACTS ""
#endif
...
int main() {
  ...
  const std::string solver_tool = ADASDF_CL_WORLD_SOLVER_CONTACTS;
  if (build_tool.empty() || solver_tool.empty()) {
    std::cout << "SKIP: world solver contacts CLI tools were not built\n";
    return 0;
  }
  const std::filesystem::path temp = ADASDF_CL_TEST_TEMP_DIR;
  std::filesystem::create_directories(temp);
  const auto model = temp / "world_solver_cli_dense.sdfbin";
  const auto samples = temp / "world_solver_samples.csv";
  const auto scene = temp / "world_solver_scene.csv";
  const auto out_csv = temp / "world_solver_contacts.csv";
  const auto report = temp / "world_solver_report.md";
  const auto stdout_txt = temp / "world_solver_stdout.txt";
  if (std::system((executableCommand(build_tool) + " " + quote(model) +
                   " --half-extent 0.5 0.5 0.5 > " +
                   quote(temp / "world_solver_build.txt"))
                      .c_str()) != 0 ||
      !writeFixtureFiles(scene, model, samples)) {
    return 1;
  }
  const std::string command =
      executableCommand(solver_tool) + " " + quote(scene) +
      " --threshold 0 --top-k 8 --max-contacts 4 --out " + quote(out_csv) +
  ...
  if (std::system(command.c_str()) != 0 ||
    ...
    std::cerr << "world solver contacts CLI failed\n";
    return 1;
  }
  std::cout << "world solver contacts CLI passed\n";
  return 0;
}
