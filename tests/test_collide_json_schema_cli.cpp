...
int main() {
  if (std::string(ADASDF_CL_BUILD_COMPRESSED_SDF).empty() ||
      std::string(ADASDF_CL_COLLIDE).empty()) {
    std::cout << "SKIP: required tools not built\n";
    return 0;
  }
  const std::filesystem::path temp = std::filesystem::path(ADASDF_CL_TEST_TEMP_DIR) / "backend_contract_collide";
  std::filesystem::create_directories(temp);
  const auto model = temp / "cube.sdfbin";
  const auto json = temp / "collide.json";
  ...
  if (runCommand(executableCommand(ADASDF_CL_COLLIDE) + " " + quotePath(model) +
                 " " + quotePath(model) +
                 " --offset 10 0 0 --json > " + quotePath(json)) != 0) {
    return 1;
  }
  const std::string text = readText(json);
  if (!containsText(text, "\"schema_id\": \"adasdf.collide.v1\"") ||
      !containsText(text, "\"status_code\": \"ADASDF_NO_COLLISION\"") ||
      !containsText(text, "\"success\": true") ||
      !containsText(text, "\"contacts\": []")) {
    std::cerr << "collide JSON contract missing expected no-collision fields\n";
    return 1;
  }
  return 0;
}
