...
#endif
#ifndef ADASDF_CL_EXPORT_COMPRESSION
#define ADASDF_CL_EXPORT_COMPRESSION ""
#endif
...
int main() {
  if (std::string(ADASDF_CL_BUILD_COMPRESSED_SDF).empty() ||
      std::string(ADASDF_CL_EXPORT_COMPRESSION).empty()) {
    std::cout << "SKIP: required tools not built\n";
    return 0;
  }
  const std::filesystem::path temp = std::filesystem::path(ADASDF_CL_TEST_TEMP_DIR) / "backend_contract_compression";
  std::filesystem::create_directories(temp);
  const auto model = temp / "cube.sdfbin";
  const auto json = temp / "compression.json";
  ...
  if (runCommand(executableCommand(ADASDF_CL_EXPORT_COMPRESSION) + " " +
                 quotePath(model) + " --json > " + quotePath(json)) != 0) {
    return 1;
  }
  const std::string text = readText(json);
  if (!containsText(text, "\"schema_id\": \"adasdf.compression.v1\"") ||
      !containsText(text, "\"rank_stats\"") ||
      !containsText(text, "\"compression_ratio\"")) {
    std::cerr << "compression JSON contract missing expected fields\n";
    return 1;
  }
  return 0;
}
