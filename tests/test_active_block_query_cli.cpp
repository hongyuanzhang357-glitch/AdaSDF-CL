...
#endif
#ifndef ADASDF_CL_ACTIVE_BLOCK_QUERY
#define ADASDF_CL_ACTIVE_BLOCK_QUERY ""
#endif
...
int main() {
  ...
  const std::string query_tool = ADASDF_CL_ACTIVE_BLOCK_QUERY;
  if (build_tool.empty() || query_tool.empty()) {
    std::cout << "SKIP: active block query CLI tools were not built\n";
    return 0;
  }
  std::filesystem::path temp = ADASDF_CL_TEST_TEMP_DIR;
  std::filesystem::create_directories(temp);
  const auto model = temp / "active_query_cli_compressed.sdfbin";
  const auto stdout_txt = temp / "active_query_cli_stdout.txt";
  const auto out_csv = temp / "active_query_cli_results.csv";
  const auto report = temp / "active_query_cli_report.md";
  const auto samples =
  ...
  if (std::system((build_cmd + " " + quote(fixture) + " " + quote(model) +
                   ...
                   " > " + quote(temp / "active_query_build.txt"))
                      .c_str()) != 0) {
  ...
  const std::string command =
      query_cmd + " " + quote(model) + " " + quote(samples) +
  ...
  if (!(rc == 0 || collisionReturnCode(rc)) ||
      ...
      !contains(readFile(out_csv), "source")) {
    std::cerr << "active block query CLI failed with code " << rc << "\n";
    return 1;
  }
  std::cout << "active block query CLI passed\n";
  return 0;
}
