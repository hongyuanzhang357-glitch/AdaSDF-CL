...
#endif
#ifndef ADASDF_CL_EXPORT_BLOCK_GRID
#define ADASDF_CL_EXPORT_BLOCK_GRID ""
#endif
...
int main() {
  if (std::string(ADASDF_CL_BUILD_COMPRESSED_SDF).empty() ||
      std::string(ADASDF_CL_EXPORT_BLOCK_GRID).empty()) {
    std::cout << "SKIP: required tools not built\n";
    return 0;
  }
  const std::filesystem::path temp = std::filesystem::path(ADASDF_CL_TEST_TEMP_DIR) / "backend_contract_block_grid";
  std::filesystem::create_directories(temp);
  const auto model = temp / "cube.sdfbin";
  const auto json = temp / "block_grid.json";
  ...
  if (runCommand(executableCommand(ADASDF_CL_EXPORT_BLOCK_GRID) + " " +
                 quotePath(model) + " --json > " + quotePath(json)) != 0) {
    return 1;
  }
  const std::string text = readText(json);
  if (!containsText(text, "\"schema_id\": \"adasdf.block_grid.v1\"") ||
      !containsText(text, "\"block_count\"") ||
      !containsText(text, "\"active_cache\"")) {
    std::cerr << "block grid JSON contract missing expected fields\n";
    return 1;
  }
  return 0;
}
