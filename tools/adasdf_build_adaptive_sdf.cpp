  ...
  try {
    ...
    for (int i = 1; i < argc; ++i) {
      ...
      if (arg == "--help" || arg == "-h") {
        ...
        options.contact_band_sampling.marker_mode = mode;
      } else if (arg == "--marker-cell-size-factor" && hasValue(i, argc)) {
        options.contact_band_sampling.marker_cell_size_factor =
            std::stod(argv[++i]);
      } else if (arg == "--marker-safety-factor" && hasValue(i, argc)) {
        options.contact_band_sampling.marker_safety_factor =
            std::stod(argv[++i]);
      } else if (arg == "--marker-min-band" && hasValue(i, argc)) {
        options.contact_band_sampling.marker_min_band = std::stod(argv[++i]);
      } else if (arg == "--marker-max-band" && hasValue(i, argc)) {
        options.contact_band_sampling.marker_max_band = std::stod(argv[++i]);
      } else if (arg == "--disable-global-halo") {
        options.contact_band_sampling.disable_global_halo = true;
      } else if (arg == "--local-halo-only") {
        options.contact_band_sampling.local_halo_only = true;
        options.contact_band_sampling.disable_global_halo = true;
        ...
        options.contact_band_sampling.coverage_audit = true;
      } else if (arg == "--coverage-samples-per-axis" && hasValue(i, argc)) {
        options.contact_band_sampling.coverage_samples_per_axis =
            std::stoi(argv[++i]);
    ...
    profiler.setToolName("adasdf_build_adaptive_sdf");
    ...
    if (dry_run) {
      ...
      progress.emit("complete", "adaptive SDF dry run complete", 1, 1);
    ...
    std::shared_ptr<adasdf::SDFModel> model;
    {
      ...
      model = adasdf::AdaptiveBlockSDFBuilder::fromSTL(
          input.string(),
          options,
          &report);
    }
    ...
    if (enable_low_rank) {
      ...
      if (!adaptive) {
        ...
        profiler.finishFailed(
            ...
            "low-rank compression requires AdaptiveBlockSDFModel");
        write_profile();
    ...
    try {
      ...
      progress.emit("write_model", "writing adaptive SDF model", 0, 1);
      std::filesystem::path temp_output = output;
      temp_output += ".tmp";
      adasdf::SDFBinWriter::write(temp_output.string(), *output_model);
    ...
    progress.emit("complete", "adaptive SDF build complete", 1, 1);
    profiler.finishCompleted();
